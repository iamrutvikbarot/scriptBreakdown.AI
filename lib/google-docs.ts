...
export async function highlightEntities(docId: string, data: ScriptData) {
  try {
    ...
    data.scenes.forEach((scene) => {
      ...
      if (scene.scene_number)
        addHighlight(
          ...
          CATEGORY_COLORS.SCENE_HEADER
        );
      if (scene.int_ext) addHighlight(scene.int_ext, CATEGORY_COLORS.INT_EXT);
      if (scene.scene_location)
        addHighlight(scene.scene_location, CATEGORY_COLORS.LOCATION);
      if (scene.time_of_day)
        addHighlight(scene.time_of_day, CATEGORY_COLORS.TIME);
      if (scene.scene_transition)
        addHighlight(scene.scene_transition, CATEGORY_COLORS.TRANSITION);
      if (scene.prod_location)
        addHighlight(scene.prod_location, CATEGORY_COLORS.PROD_LOC);
      if (scene.breakdown_name)
        addHighlight(scene.breakdown_name, CATEGORY_COLORS.ID);

      // Lists
      (scene.actors || []).forEach((a) =>
        addHighlight(a, CATEGORY_COLORS.ACTOR)
      );
      (scene.non_speaking_roles || []).forEach((r) =>
        addHighlight(r, CATEGORY_COLORS.NON_SPEAKING)
      );
      (scene.props || []).forEach((p) => addHighlight(p, CATEGORY_COLORS.PROP));
      (scene.wardrobe || []).forEach((w) =>
        addHighlight(w, CATEGORY_COLORS.WARDROBE)
      );
      (scene.set_dec || []).forEach((s) =>
        addHighlight(s, CATEGORY_COLORS.SET_DEC)
      );
      (scene.vehicles || []).forEach((v) =>
        addHighlight(v, CATEGORY_COLORS.VEHICLE)
      );
      (scene.stunts || []).forEach((s) =>
        addHighlight(s, CATEGORY_COLORS.STUNT)
      );
      (scene.sfx || []).forEach((s) => addHighlight(s, CATEGORY_COLORS.SFX));
      (scene.vfx || []).forEach((v) => addHighlight(v, CATEGORY_COLORS.VFX));
      (scene.item_quantity || []).forEach((q) =>
        addHighlight(q, CATEGORY_COLORS.QUANTITY)
      );
      (scene.additional_scheduling || []).forEach((n) =>
        addHighlight(n, CATEGORY_COLORS.NOTE)
      );
      (scene.makeup || []).forEach((m) =>
        addHighlight(m, CATEGORY_COLORS.MAKEUP)
      );

      // Cast Details
      if (scene.cast_details) {
        (scene.cast_details.age || []).forEach((a) =>
          addHighlight(a, CATEGORY_COLORS.AGE)
        );
        (scene.cast_details.gender || []).forEach((g) =>
          addHighlight(g, CATEGORY_COLORS.GENDER)
        );
        (scene.cast_details.build || []).forEach((b) =>
          addHighlight(b, CATEGORY_COLORS.BUILD)
        );
        (scene.cast_details.ethnicity || []).forEach((e) =>
          addHighlight(e, CATEGORY_COLORS.ETHNICITY)
        );
      }
    });
    ...
    if (requests.length > 0) {
      console.log(
        `Sending ${requests.length} highlight requests to Google Doc...`
      );
...
export async function extractTextFromGoogleDoc(url: string): Promise<string> {
  try {
    ...
    throw new Error(
      "Could not access Google Doc. Check permissions (Share with Service Account email)."
    );
  }
}
